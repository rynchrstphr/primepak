...
<?php 
	...
	if(isset($_POST['finalSubmitRequest']))
	{
		...
		$que = "UPDATE loanrequesttbl SET totalrequeststatus = 'chairperson approval' WHERE loanrequestid = '$loanrequestid';";
  ...
  //   	$comaker = $s['comakername'];
    	...
    	if($userLevel == "admin")
    	{
    		$message = "Good Day ! " . $memberName . ". Your Request Have been approved by the Admin. Wait for the Approval of the Chairman.";
    		$comakerid = 'Admin';
    		$qued = "INSERT INTO `notiftbl` (`id`, `memberfrom`, `date`,`title`,`message`,`details`, `memberto`, `status`) VALUES (NULL, '$comakerid', '$dateToday','Admin Approved','$message','$loanrequestid', '$memberid', 'unread')";
    	}
	...
	if(isset($_POST['cancelRequest']))
	{
	    ...
	    if($db->query($que) === true)
	    {
	      ...
	      $dateToday = date("M-d-Y h:i:s A");
	    	...
	    	if($userLevel == 'admin')  
	    	{
	    		$message = "Good Day ! " . $name . ". Sorry to inform you that your loan request have been declined by the Admin!";
	    		$que = "INSERT INTO `notiftbl` (`id`, `memberfrom`, `date`,`title`,`message`,`details`, `memberto`, `status`) VALUES (NULL, '$comakerid', '$dateToday','Admin Declined','$message','$loanrequestid', '$memberid', 'unread')";
	    	}
	    	if($userLevel == 'chairperson')
	    	{
					...
					<div class="card shadow mb-4" style="width: 90%; margin:auto;">
						<div class="card-header py-3">
							<h6 class="m-0 font-weight-bold text-primary">Loan Request | Status: <?php echo ucfirst($f['totalrequeststatus']); ?></h6>
									...
									<form class="user" action="viewrequest.php?applicationno=<?php echo $_GET['applicationno'] ?>&memberid=<?php echo $_GET['memberid']; ?>" method="post">
