...
<?php 
  ...
  if (isset($_POST['login'])) 
  {
    ...
    $query = "SELECT * from memberdetailstbl WHERE email = '".$email."' AND password = '".$password."';";
    $select = mysqli_query($db, $query);
    $fetch = mysqli_fetch_array($select);
    $row = mysqli_num_rows($select);
    if ($row == 1)  
    {
      $memberno = $fetch['memberid'];
      $_SESSION['memberid'] = $memberno;
              ...
              <div class="col-lg-6">
                <div class="p-5">
                  <div class="text-center">
                    <h1 class="h4 text-gray-900 mb-4">Primepak Multipurpose Cooperative</h1>
                  </div>
                  <form class="user" action="login.php" method="post">
                    <div class="form-group">
                      <input type="email" class="form-control form-control-user" id="email" name="email" aria-describedby="emailHelp" placeholder="Enter Email Address..." value="<?php if(isset($_POST['email'])){echo $_POST['email'];} ?>" required>
                    </div>
                    <div class="form-group">
